...
static int nb_registered_encoder = 0;
static void __iomem  * registered_encoder_mmio[3] = {0,0,0};
...
static ssize_t eqep_set_encoder(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    // Return consumed buffer count
    return count;
}
...
static ssize_t eqep_get_encoder(struct device *dev, struct device_attribute *attr, char *buf)
{
    // Get the instance structure
    ssize_t cnt = 0;
    unsigned int i;
    ...
    if (sens) {
		if (registered_encoder_mmio[0] && registered_encoder_mmio[2]) {
			local_irq_disable();
			position[0] = readl(registered_encoder_mmio[0] + QPOSCNT);
			position[2] = readl(registered_encoder_mmio[2] + QPOSCNT);
			local_irq_enable();
		}
		sens = 0;
    } else {
...
static DEVICE_ATTR(encoder,  0644, eqep_get_encoder,       eqep_set_encoder);
...
static int eqep_probe(struct platform_device *pdev)
{
    ...
    if (nb_registered_encoder < 3)

    nb_registered_encoder++;
    dev_info(&pdev->dev, "resource %x", r->start);
    switch (r->start){
    case 0x48300180:
    	encId = 0;
    	break;
